...
function setup() {
    ...
    for(let i = 0; i< 1300; i++){
        let dt = 0.02;
        let dx = speed * ( alpha * x1 - y1 * z1 ) * dt;
        let dy = speed * ( beta * y1 + x1 * z1 ) * dt;
        let dz = speed * ( delta * z1 + x1 * y1/3 ) * dt;
        x1 = x1 + dx;
        y1 = y1 + dy;
        z1 = z1 + dz;
    ...
    for(let i = 0; i< 1300; i++){
        let dt = 0.02;
        let dx = speed * ( alpha * x2- y2 * z2 ) * dt;
        let dy = speed * ( beta * y2 + x2 * z2 ) * dt;
        let dz = speed * ( delta * z2 + x2 * y2/3 ) * dt;
        x2 = x2 + dx;
        y2 = y2 + dy;
        z2 = z2 + dz;
...
function componentFX(t, x, y, z){
    return speed * ( alpha * x- y * z );//Change this function
}

function componentFY(t, x, y, z){
    return speed * ( beta * y + x * z );//Change this function
}

function componentFZ(t, x, y, z){
    return speed * ( delta * z + x * y/3 );//Change this function
}
